...
void loop()
{
  ...
  //soundSampleFromADC = soundSampleFromADC - 127;
  
  /* Overdrive
  soundSampleFromADC = soundSampleFromADC - 127
  soundSampleFromADC = (1 + badc1 / 4) * soundSampleFromADC;
  soundSampleFromADC = soundSampleFromADC + 127;

  if (soundSampleFromADC < 0)
    soundSampleFromADC = 0;
  else if (soundSampleFromADC > 255)
    soundSampleFromADC = 255;

  sramBufferSampleValue = soundSampleFromADC;
  */
  /* Flanger
  bufferIndex++;
  bufferIndex = bufferIndex % 512;
  bufferIndex2 = (bufferIndex - badc1);
  ...
  */
  ...
  soundSampleFromSramBuffer = (soundSampleFromSramBuffer - 127) * (-1);
   
  
  alphaFilter = map(badc1, 0, 255, 0, 240);
  alphaFilter = alphaFilter / 255;
  soundSampleFromSramBuffer = soundSampleFromSramBuffer * alphaFilter;
  
  soundSampleFromADC = (soundSampleFromADC - 127);
  soundSampleFromADC = (soundSampleFromADC + soundSampleFromSramBuffer) + 127;
